...
export class FilmeService {
  ...
  async listar(): Promise<ListaFilmeDTO[]> {
    ...
    return filmesListados.map(
      (filme) =>
        new ListaFilmeDTO(filme.ID, filme.NOME, filme.DURACAO, filme.SINOPSE),
    );
  }
  ...
  async inserir(dados: criaFilmeDTO): Promise<RetornoCadastroDTO> {
    ...
    filme.genero = await this.generoService.localizarGenero(dados.GENERO);
