...
export class SerieController {
  ...
  @ApiResponse({
    status: 500,
    description: 'Retorna que houve erro na inclusão.',
  })
  @ApiResponse({
    status: 400,
    description: 'Retorna que há algum dado inválido na requisição.',
  })
  async criaSerie(
    @Body() dadosSerie: criaSerieDTO,
  ...
  @ApiResponse({
    status: 200,
    description: 'Retorna que houve sucesso na alteração',
  })
  @ApiResponse({
    status: 500,
    description: 'Retorna que houve erro na alteração.',
  })
  @ApiResponse({
    status: 400,
    description: 'Retorna que há algum dado inválido na requisição.',
  ...
  async alteraSerie(
    @Body() dadosNovos: alteraSerieDTO,
    @Param('id') id: string,
  ) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Retorna que houve sucesso na exclusão',
  })
  @ApiResponse({
    status: 500,
    description: 'Retorna que houve erro na exclusão.',
  })
  async removeSerie(@Param('id') id: string) {
    //aqui é definido que vai receber dados da URL(param)
  ...
  @ApiResponse({
    status: 200,
    description: 'Retorna que houve sucesso na consulta',
  })
  @ApiResponse({
    status: 500,
    description: 'Retorna que houve erro na consulta.',
  })
